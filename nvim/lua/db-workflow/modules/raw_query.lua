...
function M.execute(opts)
    ...
    utils.notify("Выполняем raw SQL запрос: " .. line_count .. " строк")
    ...
    local output, err = db_executor.execute_query(selected_text, { raw = true })
    if not output then
        utils.error(err)
        return
    end

    result_display.show(output, "raw_sql_query_result", "sql")
end
