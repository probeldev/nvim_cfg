...
function M.show()
	...
	telescope_menu.show_table_picker(actions, "🗃️  Выберите процедуру", function(selected_action)
...
function M.get_available_actions()
    local menu_output, err = utils.execute_system_command(config.get_command() .. ' -procedures', "")
...
function M.run_action(action_name)
    local command = config.get_command() .. " -procedure=" .. action_name
